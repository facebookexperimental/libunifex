...
in-flight sender-based async operations as an extension to P0443R13.

The paper [P0443R13](https://wg21.link/P0443R13) proposes adding the sender/receiver concepts
...
While P0443R13 provides the ability for an async operation to complete with an empty
...
* Impacts on sender-based algorithms in [P1897](https://wg21.link/P1897) which
...
* Impacts on  `std::task/lazy` propsed in in [P1056](https://wg21.link/P1056) to
...
The first example is the `when_all()` algorithm, proposed in [P1897](https://wg21.link/P1879).
...
then the whole operation completes with the first such result. However, completing with
...
The paper [P0443R13](https://wg21.link/P0443R13) adds a basis operation named `schedule()`
...
async accept operation which would otherwise never complete.
...
it easier to reason about control-flow compared to `goto`. These control-flow
constructs can be composed arbitrarily to create complex logic and algorithms.
...
already computed its result and the results of those operations will just be
discarded.
...
For example, the `basic_waitable_timer` type described in N4734 (C++ Extensions for Networking Working Draft)
provides a `.cancel()` method that will cancel all outstanding `async_wait()` operations
launched on the timer object.
...
to report on a failure to request cancellation.
...
list of outstanding async operations itself as an epoll implementation does.
...
There would therefore be a small amount of additional complexity/overhead to
support this.
...
other calls to start async operations avoids any synchronisation overhead
that would be required if cancellation were able to be requested concurrently
from other threads. This synchronisation burden is typically placed on the strand
executor used to serialise access to the object.
...
the async model was greatly expanded in v4.0.
...
to the standard library in C++20.
...
is a token for which cancellation can never be requested.
...
For example:
```c++
task<int> another_operation();

task<void> consumer() {
  // Immediately co_awaiting the returned task has two implications:
  ...
  int x = co_await another_operation();
...
cancellation-token (or `std::stop_token`) into the child operation to
...
task<void> consumer(cancellation_token ct) {
  // Explicitly passing the stop-token to child operations allows us to
  // communicate cancellation requests of the parent operation through
  // to child operations.
...
task<Result> timeout(AwaitableFactory f, Scheduler s, Duration d, cancellation_token ct = {}) {
  cancellation_source src;
  cancellation_registration cancelParent{std::move(ct), [&] { src.request_cancellation(); }};
...
In C++20 we added three new types: `std::stop_token`, `std::stop_source` and `std::stop_callback<CB>`.
...
Note: See [P1895R0](https://wg21.link/P1895R0) "tag_invoke: A general pattern for supporting
customisable functions" for details of the proposed `std::tag_invoke()`
...
* Technically the token is not "stoppable" but rather, "able to produce a stop request".
  ...
  what this concept is checking for.
...
from the thread resource and destroy the `std::stop_source` before the thread
has finished using the `std::stop_token`.

This shared-ownership model implies some runtime overhead, however, typically
involving heap-allocation and atomic-reference counting of the shared state.
...
thus will observe any stop-requests send to the parent operation.
...
Generalising this forwarding mechanism for receiver queries is a
topic for another paper, however.
...
See Appendix C for details of the implementation.
...
2. Where a `task` coroutine awaits a sender we need to make sure that the
   `task` injects a receiver into the call to `connect()` on the sender that
...
This prototype also explores some strategies for representing a 'done' result
...
and in non-coroutine functions have been explored in [P1677](https://wg21.link/P1677)
...
* allocator customisation - this is also explored by the paper [P1681](https:/wg21.link/P1681)
...
revision of P1056. The author does not believe that P1056R1 should be accepted
...
a stop-request which returns an operation-state that takes a more efficient approach,
...
always hold the last reference to the shared stop-state.
...
if desired, as it is also a valid implementation of
...
## Appendix A: Cancellable `schedule()` operation

TODO

## Appendix B: The `transform()` algorithm is transparent to cancellation

TODO

## Appendix C: The `stop_when()` algorithm

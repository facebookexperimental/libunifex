...
public:
  ...
  int get_count() { return run_count_; }
...
struct FuncWrapper {
  T func_;
  operator std::invoke_result_t<T>() { return func_(); }
};
...
public:
  ...
  delegating_operation(InitFunc&& func, delegating_context* context)
    : op_{std::in_place_type_t<remove_cvref_t<decltype(func())>>{},
...
public:
  ...
  struct LocalContextType {
    ...
    inline void start() noexcept { op_.start(); }
  };

  using SchedSender = schedule_result_t<
      decltype(UNIFEX_DECLVAL(timed_single_thread_context&).get_scheduler())>;

  template <typename Receiver>
  using operation = delegating_operation<
      connect_result_t<
...
class delegating_scheduler {
public:
  auto schedule() const noexcept { return delegating_sender{context_}; }

  [[maybe_unused]] friend bool
  operator==(delegating_scheduler, delegating_scheduler) noexcept {
    return true;
  }
  [[maybe_unused]] friend bool
  operator!=(delegating_scheduler, delegating_scheduler) noexcept {
...
int main() {
  ...
  std::printf(
      "inner_delegating_ctx operations: %d\n",
      inner_delegating_ctx.get_count());
  std::printf(
      "outer_delegating_ctx operations: %d\n",

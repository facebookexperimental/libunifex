...
struct _receiver<Receiver>::type {
  ...
  void set_value(Values&&... values) && {
    unifex::set_value((Receiver &&)(receiver_),
        std::variant<std::tuple<remove_cvref_t<Values>...>>(
        std::make_tuple((Values &&)(values)...)));
  }
...
struct _sender<Predecessor>::type {
  ...
  using value_types = Variant<
      Tuple<sender_value_types_t<Predecessor, std::variant, std::tuple>>>;
  ...
  -> connect_result_t<member_t<Sender, Predecessor>, receiver_t<remove_cvref_t<Receiver>>> {
    return unifex::connect(static_cast<Sender&&>(s).pred_,
...
public:
  ...
  noexcept(std::is_nothrow_constructible_v<_into_variant::sender<Sender>, Sender>)
  -> _result_t<Sender> {
    return _into_variant::sender<Sender>{(Sender &&)(predecessor)};
  }
  constexpr auto operator()() const
  noexcept(is_nothrow_callable_v<tag_t<bind_back>, _fn>) -> bind_back_result_t<_fn> {

...
private:
  struct waiter_base {
    bool enqueued() const noexcept { return next_ != this; }

    void set_dequeued() noexcept { next_ = this; }
...
void async_mutex::lock_sender::_op<Receiver>::type::start() noexcept {
  ...
  if (enqueued_state_.exchange(
          enqueued_state::kLocking, std::memory_order_acq_rel) ==
      enqueued_state::kCancelled) {
    // stop callback ran synchronously
    stop_callback_.reset();
    enqueued_state_.store(
        enqueued_state::kCancelled, std::memory_order_release);
  ...
  if (enqueued_state_.compare_exchange_strong(
    ...
    if (enqueued) {
      // we need to return here; the next state transition will happen when an
      // unlocker dequeues us or the stop request fires, both of which could be
      // happening concurrently to this code so it's unsafe to touch any members
      // at this point
      return;
    } else {
      // we took the lock synchronously, so no unlocker will wake us up, but the
      // stop request could run
      stop_callback_.reset();
    ...
    if (oldState == enqueued_state::kCancelled) {
      // the stop request ran; we have to handle the possibility that we were
      // enqueued onto the list of waiters and then popped by an unlocker
      if (!enqueued || !mutex_.try_dequeue(this)) {
        // we got the lock but don't intend to keep it so give it back
        mutex_.unlock();
      }
    } else {
      // we were enqueued but an unlocker dequeued us, giving us the lock but
      // not completing
...
template <typename Receiver>
void async_mutex::lock_sender::_op<
    Receiver>::type::stop_callback::operator()() noexcept {
  self->set_done();
}
...
void async_mutex::lock_sender::_op<Receiver>::type::set_done() noexcept {
  ...
  // we transitioned to cancelled; whether there's more to do depends on what
  // from
  switch (oldState) {
...
void async_mutex::lock_sender::_op<Receiver>::type::set_value() noexcept {
  ...
  // we transitioned to unlocked; whether there's more to do depends on what
  // from
  if (oldState == enqueued_state::kLocking) {
    // start() is still running
    return;
  } else if (oldState == enqueued_state::kEnqueued) {
    // start() has returned or will soon and we beat the stop request to
    // updating state
    stop_callback_.reset();
    forwardingOp_.start(*this);
  } else {

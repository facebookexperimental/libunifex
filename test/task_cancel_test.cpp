      ...
      then(
          ...
          [](auto&&... values) {
            return std::optional<value_type>{
                std::in_place, static_cast<decltype(values)>(values)...};
          }),
...
TEST(TaskCancel, PropagatesStopToken) {
  ...
  std::optional<inplace_stop_token> i = sync_wait(with_query_value(
...
TEST(TaskCancel, StopIfRequested) {
  inplace_stop_source stopSource;
  std::optional<int> i = sync_wait(with_query_value(
      with_scheduler_affinity(
          test_stop_if_requested(stopSource), current_scheduler),
      get_stop_token,
      stopSource.get_token()));
  EXPECT_TRUE(!i);

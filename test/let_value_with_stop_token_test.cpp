...
struct InplaceStoppableIntReceiver : public UnstoppableSimpleIntReceiver {
    ...
    InplaceStoppableIntReceiver(inplace_stop_source& source) noexcept : source_(source) {}

    friend
    inplace_stop_token tag_invoke(tag_t<get_stop_token>, const InplaceStoppableIntReceiver& r) noexcept {
...
struct NonInplaceStoppableIntReceiver : public UnstoppableSimpleIntReceiver {
    ...
    NonInplaceStoppableIntReceiver(inplace_stop_source& source) noexcept : source_(source) {}

    friend
    inplace_stop_token_redux tag_invoke(tag_t<get_stop_token>, const NonInplaceStoppableIntReceiver& r) noexcept {
...
TEST(LetWithStopToken, SimpleInplaceStoppableNoexcept) {
  static_assert(std::is_same_v<stop_token_type_t<InplaceStoppableIntReceiver>, inplace_stop_token>);

  // Simple usage of 'let_value_with_stop_token()' with receiver with inplace stop token
  // - Sets up some work to execute when receiver is cancelled
  int external_context = 0;
  inplace_stop_source stopSource;
  auto stop_token_functor = [&](auto stopToken) noexcept {
            return let_value_with([stopToken, &external_context]() noexcept {
                auto stopCallback = [&]() noexcept { external_context = 42; };
                using stop_token_t =
                    unifex::remove_cvref_t<decltype(stopToken)>;
                using stop_callback_t =
  ...
  static_assert(is_nothrow_connectable_v<callable_result_t<decltype(let_value_with_stop_token), decltype(stop_token_functor)>, InplaceStoppableIntReceiver>);
  auto op = unifex::connect(
        let_value_with_stop_token(std::move(stop_token_functor)), InplaceStoppableIntReceiver{stopSource});
  unifex::start(op);
...
TEST(LetWithStopToken, SimpleUnstoppable) {
  static_assert(is_stop_never_possible_v<stop_token_type_t<UnstoppableSimpleIntReceiver>>);

  // Simple usage of 'let_value_with_stop_token()' with receiver with unstoppable stop token
  // - Sets up some work to execute when receiver is cancelled
...
TEST(LetWithStopToken, SimpleNonInplaceStoppable) {
  static_assert(!std::is_same_v<stop_token_type_t<NonInplaceStoppableIntReceiver>, inplace_stop_token>);

  // Simple usage of 'let_value_with_stop_token()' with receiver with stop token that's stoppable but not inplace stop token
  // - Sets up some work to execute when receiver is cancelled
  int external_context = 0;
  inplace_stop_source stopSource;
  auto op = unifex::connect(

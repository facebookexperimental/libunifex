...
TEST(UponDone, StaticTypeCheck) {
  ...
  static_assert(std::is_same_v<decltype(res5)::value_types<variant, std::tuple>,
      variant<std::tuple<int>>>);

  auto res6 = just_done()
    | upon_done([]{
      });
  static_assert(std::is_same_v<decltype(res6)::value_types<variant, std::tuple>,
      variant<std::tuple<>>>);
  ...
  auto sch_then_sender = schedule_after(scheduler, 200ms)
    | then([]{return 2;});
  static_assert(std::is_same_v<decltype(sch_then_sender)::value_types<variant, std::tuple>,
      variant<std::tuple<int>>>);

  auto res7 = sch_then_sender
    | upon_done([]{});
  static_assert(std::is_same_v<decltype(res7)::value_types<variant, std::tuple>,
      variant<std::tuple<int>, std::tuple<>>>);

  auto res8 = std::move(sch_then_sender)
    | upon_done([]{ return 1.2; });
  static_assert(std::is_same_v<decltype(res8)::value_types<variant, std::tuple>,

...
struct ManagedDestroyResource : public ResourceBase {
  ...
  ~ManagedDestroyResource() noexcept { EXPECT_TRUE(destroyCalled_); }

private:
  bool destroyCalled_{false};
};

task<void> managed_async_destroy(AsyncResourceTest* f) {
  // drop immediately
  (void)co_await make_async_resource(
      f->ctx.get_scheduler(), f->outerScope, [f](auto, auto) noexcept {
        return ManagedDestroyResource{&(f->objectCount)};
      });
...
TEST_F(AsyncResourceTest, managed_async_destroy) {
  sync_wait(managed_async_destroy(this));
}
#endif

...
TEST_F(async_mutex_v2_test, cancel_awaiting) {
  bool first_acquired, first_finished, second_acquired,
       second_finished, second_cancelled;

  sync_wait(when_all(
    critSecTask(first_acquired, first_finished),
    stop_when(
      critSecTask(second_acquired, second_finished) | maybeCancelled(second_cancelled),
...
TEST_F(async_mutex_v2_test, cancel_running) {
  bool first_acquired, first_finished, first_cancelled,
       second_acquired, second_finished;

  sync_wait(when_all(
    stop_when(
      critSecTask(first_acquired, first_finished) | maybeCancelled(first_cancelled),
      schedule_after(timer(), 100ms)),
...
TEST_F(async_mutex_v2_test, cancel_awaiting_middle) {
  bool first_acquired, first_finished, second_acquired,
       second_finished, second_cancelled, third_acquired,
       third_finished;
...
TEST_F(async_mutex_v2_test, cancel_early) {
  ...
  sync_wait(stop_when(
      delayedCritSecTask(first_acquired, first_finished) | maybeCancelled(first_cancelled),
      schedule_after(timer(), 100ms)));
...
TEST_F(async_mutex_v2_test, cancel_awaiting_early) {
  ...
  sync_wait(when_all(
    critSecTask(first_acquired, first_finished),
    stop_when(
      delayedCritSecTask(second_acquired, second_finished) | maybeCancelled(second_cancelled),
      schedule_after(timer(), 100ms))));
